...
aggregateScorers playerNames =
    let
        incrementCounts playerName scores =
            Dict.update playerName incrementCount scores
    in
    List.foldl incrementCounts Dict.empty playerNames
...
resetPlayerGoalCount playerName playerGoalCounts =
    let
        setToZero _ =
            Just 0
    in
    Dict.update playerName setToZero playerGoalCounts
